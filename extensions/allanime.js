...
class _Src extends AnimeSource {
  constructor(){super({id:'allanime',name:'AllAnime',lang:'en',baseUrl:'https://allmanga.to',icon:'🎬',version:'2.0',description:'AllAnime - GraphQL API, most reliable'});}
  async _q(query,vars){
    const r=await $.fetch('https://api.allanime.day/api?variables='+encodeURIComponent(JSON.stringify(vars||{}))+'&query='+encodeURIComponent(query),{headers:{'User-Agent':'Mozilla/5.0','Referer':'https://allanime.to','Accept':'application/json'}});
    if(r.status===200&&r.data)return JSON.parse(r.data);
    return null;
  }
  ...
  async getEpisodes(id){
    const d=await this._q('query($id:String!){show(_id:$id){availableEpisodes{sub dub}lastEpisodeInfo{sub{episodeString}dub{episodeString}}}}',{id});
    if(!d||!d.data||!d.data.show)return[];
    const n=d.data.show.availableEpisodes;
    const count=Math.max((n&&n.sub)||0,(n&&n.dub)||0);
    if(!count)return[];
    const eps=[];
    for(let i=1;i<=Math.min(count,2000);i++)eps.push(new Episode({id:id+'__'+i,title:'Episode '+i,number:i}));
    return eps;
  }
  async getVideos(id,eid){
    ...
    for(const src of d.data.episode.sourceUrls){
      try{
        ...
        if(!raw)continue;
        // Strip --gg-- and --ani-- prefixes
        raw=raw.replace(/^--gg--|^--ani--|^--s--|^--a--/,'');
        // Resolve clock:// protocol
        if(raw.startsWith('clock://')){raw='https://'+raw.slice(8);}
        // If it's a clock/CDN resolve URL, fetch it to get actual stream links
        if(raw.includes('.ebb.lat/')||raw.includes('allanime.co/clock')||raw.includes('/clock')){
          try{
            const cr=await $.fetch(raw,{headers:{'User-Agent':'Mozilla/5.0','Referer':'https://allanime.to'}});
            if(cr.status===200&&cr.data){
              const cd=JSON.parse(cr.data);
              if(cd.links&&cd.links.length){
                for(const l of cd.links){
                  const url=l.link||l.url||l.src||'';
                  if(url&&(url.startsWith('http')||url.startsWith('//'))){
                    links.push(new VideoLink({url:url.startsWith('//')?'https:'+url:url,quality:l.resolutionStr||l.resolution||'Auto',isM3U8:url.includes('.m3u8')}));
                  }
                }
                continue;
              }
        ...
        // Direct URL
        if(raw.startsWith('http')&&(raw.includes('.m3u8')||raw.includes('.mp4')||raw.includes('cdn'))){
          links.push(new VideoLink({url:raw,quality:'Auto',isM3U8:raw.includes('.m3u8')}));
        }
      }catch(e){}
    }
