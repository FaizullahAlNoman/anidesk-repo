...
class _Src extends AnimeSource {
  constructor(){super({id:'anizone',name:'AniZone',lang:'en',baseUrl:'https://anizone.to',icon:'🏯',version:'1.0'});}
  ...
  async getVideos(id,eid){const d=await _get('/watch/'+encodeURIComponent(eid));if(d&&d.sources)return d.sources.map(s=>new VideoLink({url:s.url,quality:s.quality||'Auto'}));return[];}
}
if(typeof ExtManager!=='undefined'&&!ExtManager.get('anizone'))ExtManager.register(new _Src());
})();
